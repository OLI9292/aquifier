...
class Train extends Component {
  ...
  formatLevels(allLevels, userLevels) {
    const completedLevels = _.filter(allLevels, level => {
      ...
      return (
        uLvl &&
        (level.type === "speed" || uLvl.progress.length >= level.progressBars)
      )
    })
    ...
    const open = completed.length
      ? _.range(1, Math.max(...completed) + 1).concat(_.last(completed) + 1)
      : [1]
  ...
  imgSrc(level, images) {
    ...
    const imgKey = _.find(_.keys(images), k =>
      k.includes(level.name.split(" ")[0])
    )
    return images[imgKey || "default.png"]
  }

  ladders() {
    const ladders = _.uniq(
      _.pluck(_.flatten(_.values(this.state.levels)), "ladder")
    ).sort((a, b) => a - b)
    _.forEach(_.clone(ladders), num => {
      ...
      if (exists) {
        ladders.splice(index, 0, speedKey)
      }
    })
    return ladders
  }
    ...
    const levelComponents = (() => {
      ...
      return _.map(ladders, ladder => {
          ...
          level.avgAcc =
            ...
            parseInt(
              parseFloat(
                _.reduce(
                  userLevel.progress,
                  (acc, curr) => acc + curr.bestAccuracy,
                  0
                ) * 100
              ) / parseFloat(Math.max(userLevel.progress.length, 0)),
              10
            )
          level.bestScore =
            level.type === "speed" &&
            level.completed &&
            userLevel.progress[0].bestScore
          level.mastered = level.avgAcc === 100

          if (level.type === "speed") {
            const index =
              parseInt(
                _.filter(ladders, l => String(l).includes("speed")).indexOf(
                  ladder
                ),
                0
              ) + 1
        ...
        return levels.length > 1 ? (
          <MultipleLevelContainer key={ladder}>{levels}</MultipleLevelContainer>
        ) : (
          levels
        )
      })
