...
class Game extends Component {
  ...
  setQuestion() {
    ...
    if (question) {
      ...
      if (_.contains(["multiplayer", "speed"], this.props.type)) {
        ...
        this.setState(
          { questions: questions, questionIndex: questionIndex },
          this.setQuestion
        )
      } else {
        this.gameOver()
  ...
  gameOver() {
    ...
    const time = Math.floor(
      moment.duration(moment().diff(gameStartTime)).asSeconds()
    )
  ...
  guessed(choice, idx) {
    ...
    if (correct) {
      this.setState(
        { question: _.extend({}, this.state.question, {}, { answer: answer }) },
        this.checkComplete
      )
    } else {
  ...
  checkComplete() {
    ...
    const correct =
      this.state.incorrectGuesses === 0 && this.state.hintCount === 0
    const seconds = Math.ceil(
      moment.duration(moment().diff(questionStartTime)).asSeconds()
    )
    const isSpeedy = seconds < 4
    const nextQuestionIndex =
      questionIndex +
      (isSpeedy && _.contains(["train", "explore"], type) ? 2 : 1)
  ...
  glowAnswer(giveAnswer) {
    ...
    const idx = _.findIndex(q.choices, c =>
      caseInsEq(c.value, nextCorrect.value)
    )
  ...
  render() {
    ...
    const { battleUsernames, mobile } = this.props
    ...
    const promptText = () => {
      ...
      return _.map(prompt, (section, i) => {
        ...
        const style = highlight
          ? { color: color.warmYellow, fontFamily: "BrandonGrotesqueBold" }
          : {}
        return (
    ...
    const answerSpace = (value, missing, idx) => {
      ...
      return (
        <AnswerSpace key={idx} margin={margin}>
          <AnswerValue missing={missing}>{value}</AnswerValue>
          <Underline
            color={
              questionComplete
                ? color.warmYellow
    ...
    const gridTemplates = (count, isSpellQuestion) => {
      ...
      const gridStr = c =>
        _.map(Array(c).fill(100 / c), perc => `${perc}%`).join(" ")
      ...
      return isSpellQuestion
        ? {
            mobile: {
              column: gridStr(count === 16 ? 4 : 4),
              row: gridStr(count === 16 ? 4 : 3)
            },
            regular: twoByX
          }
        : count === 4
    ...
    const questionComponents = (() => {
      ...
      const prompt = (
        <PromptContainer>
          <Prompt>
            <PromptValue>{promptText()}</PromptValue>
          </Prompt>
        </PromptContainer>
      )

      const showKoreanWord =
        this.props.type === "korea-demo" && questionComplete
      ...
      const answer = (
        <Answer>
          ...
          {_.map(question.answer, (a, idx) =>
            answerSpace(a.value, a.missing, idx)
          )}
        </Answer>
      )

      const buttonGrids = gridTemplates(
        question.choices.length,
        isSpellQuestion
      )

      const choices = (
        <Choices grid={buttonGrids.regular} mobileGrid={buttonGrids.mobile}>
          {_.map(question.choices, (c, idx) =>
            choiceButton(c, idx, question.choices.length)
          )}
        </Choices>
      )

      // TODO: - check all these conditions
      const interlude = !_.contains(["battle", "speed"], this.props.type) && (
        <Interlude
    ...
    const levelInfo = (
      <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        <div>
          {_.has(this.props.level, "progress") &&
            _.map(_.range(1, this.props.level.progress[1] + 1), n => {
              return (
                <StageDot key={n} green={n <= this.props.level.progress[0]} />
              )
            })}
        </div>
      </div>
    ...
    return (
      <Content opacity={gameOpaqueness}>
        <div style={{ width: "100%", height: "100%" }}>
          ...
          <Bottom>
            {questionComplete &&
              question.word && <Audio word={question.word} />}
            {levelInfo}
            {helpButton}
          </Bottom>
        </div>
