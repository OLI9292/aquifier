...
import { fetchStudentsAction, updateClassAction } from "../../Actions/index"
...
class MyClass extends Component {
  ...
  loadClass(props) {
    const id =
      props.user &&
      get(_.find(props.user.classes, c => c.role === "teacher"), "id")
    if (!id || this.state.loadingClass) {
      return
    }
  ...
  nameObj(str) {
    return {
      ...
      lastName: _.rest(str.split(" ")).join(" ")
    }
  }
  ...
  createStudents = async () => {
    const { user, students, session } = this.props

    const id = get(_.find(user.classes, c => c.role === "teacher"), "id")
    const newStudents = _.filter(
      _.map(this.state.newStudents, this.nameObj),
      obj => obj.firstName.length && obj.lastName.length
    )
    const data = { students: newStudents, email: user.email }
    if (!id || !session || _.isEmpty(data.students) || !data.email) {
      return
    }

    const result = await this.props.dispatch(
      updateClassAction(id, data, session)
    )
  ...
  render() {
    ...
    const showDistrictAdminView =
      get(this.props.user, "email") === "testadmin@gmail.com"
    const showTestTeacherView =
      get(this.props.user, "email") === "test@gmail.com"
    ...
    const students = showDistrictAdminView
      ...
      : showTestTeacherView
      ? _.filter(this.props.students, student =>
          ["Mary", "Jack", "Bertie", "Amelia"].includes(student.firstName)
        )
      : this.props.students
      ...
      <Container>
        ...
        <div style={{ display: addingStudents ? "" : "none" }}>
          <ModalContainer>
            ...
            <Button.medium
              style={{
                display: isImporting ? "none" : "inline-block",
