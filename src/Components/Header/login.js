...
class Login extends Component {
  ...
  validLogin() {
    if (!this.state.email.length) {
      this.setState({ isError: true, message: "Email missing." })
    } else if (!this.state.password.length) {
      this.setState({ isError: true, message: "Password missing." })
    } else {
      return true
    }
  }
    ...
    return (
      <Container>
        <MobileExit
          ...
          display={"fixed"}
        />
        <Header.small>login</Header.small>
        ...
        <div style={{ marginTop: "25px", marginBottom: "25px" }}>
          ...
          <input
            style={_.extend({}, InputStyles.default, {
              width: "100%",
              marginTop: "10px"
            })}
            type={"password"}
        ...
        <p
          style={{
            marginTop: "0",
            color: this.state.isError ? color.red : color.green
          }}
        >
          {this.state.message}
        </p>
